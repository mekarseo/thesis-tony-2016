...
class TalentTypeModel extends ActiveRecord
{
    public $counter;
    public $faculty;
    public $talent_type;
    ...
    public function search($params)
    {
        ...
        $query = $this->find();
        $query->alias('a');
        $query->select(['COUNT(*) as counter', 'b.faculty as faculty', 'c.talent_type as talent_type']);
        $query->joinWith(['education', 'talent']);
        $query->andFilterWhere(['like','a.term', $this->term]);
        $query->where(['IS NOT','a.std_id', NULL]);
        $query->groupBY(['c.talent_type', 'b.faculty']);
        $query->orderBY(['c.talent_type' => SORT_ASC, 'b.faculty' => SORT_ASC]);
        return $query;
    }
    ...
    public function getRow($model)
    {
        ...
        foreach ($model->all() as $tmp) {
            ...
            $column[$tmp->talent_type] = $head_tmp;
        }

        foreach ($model->all() as $tmp) {
            $column[$tmp->talent_type][$tmp->faculty] = $tmp->counter;
        }
    ...
    {
        return $this->hasOne(UserTalent::className(), ['id' => 'id'])->alias('c')->where(['!=','c.talent_type', ''])->orWhere(['!=', 'c.talent_type', '']);
    }
}
