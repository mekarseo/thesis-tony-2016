...
class AccountController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if (!$model->passwordCheck()) {
                Yii::$app->session->setFlash('error', Yii::t('app', 'Old Password not match.'));

                return $this->redirect(['/account/account']);
            }

            if ($model->validate() && $model->update()) {
                Yii::$app->session->setFlash('success', Yii::t('app', 'Account has been updated.'));
                
                return $this->redirect(['/account/account']);
            }

            Yii::$app->session->setFlash('error', Yii::t('app', 'Account has not been updated.'));
        }
    ...
    {
        // Text
        $data['t_title']    = Yii::t('app', 'Manage Account');
