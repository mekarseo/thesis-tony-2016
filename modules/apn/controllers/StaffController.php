...
use app\modules\apn\models\StaffModel;
...
HTML;
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->validate() && $model->create()) {
                Yii::$app->session->setFlash('success', Yii::t('app', 'Staff has been created'));
                
                return $this->redirect(['/apn/staff']);
            }

            Yii::$app->session->setFlash('error', Yii::t('app', 'Staff has been duplicated'));
        }
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->validate() && $model->update()) {
                Yii::$app->session->setFlash('success', Yii::t('app', 'Staff has been updated'));
                
                return $this->redirect(['/apn/staff']);
            }

            Yii::$app->session->setFlash('error', Yii::t('app', 'Staff has not been updated'));
        }
    ...
    public function actionDelete()
    {
        if (Yii::$app->request->isPost && Yii::$app->request->post('selected') != null) {
            ...
            Yii::$app->session->setFlash('success', Yii::t('app', 'Staff has been deleted'));
        }
    ...
    public function getList()
    {
        ...
        // Text
        $data['t_title']    = Yii::t('app', 'Staff');
        $data['t_confirm']  = Yii::t('app', 'Are You Sure?');
        $data['t_create']   = Yii::t('app', 'Create');
        $data['t_delete']   = Yii::t('app', 'Delete');
        // Link
        $data['l_create']   = Url::to(['/apn/staff/create']);
        $data['l_delete']   = Url::to(['/apn/staff/delete']);
    ...
    public function getForm()
    {
        ...
        // Link
        $data['l_cancel']   = Url::to(['/apn/staff']);
        ...
        Yii::$app->view->title = $data['t_title'];
        Yii::$app->view->params['breadcrumbs'][] = ['label' => Yii::t('app', 'Admin Panel'), 'url' => ['/apn']];
        Yii::$app->view->params['breadcrumbs'][] = ['label' => Yii::t('app', 'Staff'), 'url' => ['/apn/staff']];
        Yii::$app->view->params['breadcrumbs'][] = $data['t_title'];
    ...
    {
        if (($model = StaffModel::findOne($id)) !== null) {
            return $model;
        } else {
