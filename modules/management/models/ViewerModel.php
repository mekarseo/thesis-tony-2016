...
class ViewerModel extends ActiveRecord
{
    /**
     * @property
     */
    ...
    {
        $query = $this->find();
        $query->where(['id' => $this->getViewers()]);
    ...
    public function create()
    {
        ...
        if ($query == null) {
            ...
            Yii::$app->db->createCommand("INSERT INTO {{%auth_assignment}} SET `item_name` = 'Viewer', `user_id` = " . (int)$user_id . ", `created_at` = '" . time() . "';")->execute();
    ...
    {
        $query = Yii::$app->db->createCommand("SELECT * FROM {{%auth_assignment}} WHERE `item_name` = 'Viewer';")->queryAll();
